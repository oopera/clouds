...
async function InitializeScene() {
  ...
  pipeline[0].bindings[0] = CreateBindGroup(
    device,
    pipeline[0].pipeline,
    earthBindings
  );
  ...
  pipeline[1].bindings = [
    CreateBindGroup(device, pipeline[1].pipeline, cloudBindings),
    CreateBindGroup(device, pipeline[1].pipeline, [
      {
        binding: 0,
        resource: {
  ...
  pipeline[2].bindings = [
    CreateBindGroup(device, pipeline[2].pipeline, cloudBindings),
    CreateBindGroup(device, pipeline[2].pipeline, [
      {
        binding: 0,
        resource: {
  ...
  pipeline[3].bindings = [
    CreateBindGroup(device, pipeline[3].pipeline, cloudBindings),
    CreateBindGroup(device, pipeline[3].pipeline, [
      {
        binding: 0,
        resource: {
  ...
  pipeline[4].bindings = [
    CreateBindGroup(device, pipeline[4].pipeline, atmosphereBindings),
  ];
  ...
  function draw() {
    ...
    function setVertexBuffers(renderPass: any): void {
      for (let i = 0; i < 3; i++) {
        renderPass.setVertexBuffer(i, buffers[0][i]);
      }
    }

    for (let i = 0; i < pipeline.length; i++) {
      renderPass.setPipeline(pipeline[i].pipeline);
      setVertexBuffers(renderPass);
      pipeline[i].bindings.forEach((bindGroup, index) => {
