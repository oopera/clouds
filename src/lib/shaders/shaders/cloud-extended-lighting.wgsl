...
fn calculate_height(min_layer_sphere_radius: f32, max_layer_sphere_radius: f32, noise: vec4<f32>, detail_noise: vec4<f32>, coverage: f32, percent_height: f32, scaling_factor: f32) -> vec2<f32> {
  ...
  detail_modifier = detail_modifier * pow(coverage, 2);
...
@fragment fn fs(output: Output) -> @location(0) vec4<f32> {
  var output_color: vec3<f32> = vec3<f32>(0.62, 0.63, 0.65);
  ...
  for (var i: f32 = 0.0; i < steps; i += 1.0) {
    ...
    cloud_density += clamp(getCoverage(current_point, distance_to_center, length(current_point - inner_sphere_point), samples, true) * cloudUniforms.density, 0.0, 1.0);
  ...
  // output_color += sun_transmittance * sun_color  * light;
   output_color += sun_output; 
