...
fn calculate_density(noise: vec4<f32>, detail_noise: vec4<f32>, percent_height: f32) -> f32{
  var shape_noise: f32;

  shape_noise  = noise.r * 0.625 + noise.b * 0.25 + noise.a * 0.125;

  shape_noise = -(1 - shape_noise);
  shape_noise = ReMap(noise.r, shape_noise, 1.0, 0.0, 1.0);

  var detail = detail_noise.r * 0.625 + detail_noise.g * 0.25 + detail_noise.b * 0.125;
...
fn calculate_height(min_layer_sphere_radius: f32, max_layer_sphere_radius: f32, noise: vec4<f32>, detail_noise: vec4<f32>, coverage: f32, percent_height: f32, scaling_factor: f32) -> vec2<f32> {
  ...
  var maxheight = ReMap(pow(final_density, 1), 0.0, 1.0, min_layer_sphere_radius, (max_layer_sphere_radius - sphere_radius));
...
fn getSamples(inner_sphere_point:vec3<f32>, sphere_uv: vec2<f32>) -> Samples {
  ...
  var detail_noise = textureSample(noise_texture, noise_sampler, inner_sphere_point * lod * 2);
...
fn getCoverage(current_point: vec3<f32>, distance_to_center: f32, distance_to_inner_sphere:f32, samples: Samples, reverse: bool) -> f32 {
...
fn getScale(current_point: vec3<f32>, distance_to_center: f32) -> f32{
  var distance_low = ReMap(length(sphere_center - current_point), layer_1_offset, layer_1_buffer, 0.0, 1.0);
...
@fragment fn fs(output: Output) -> @location(0) vec4<f32> {
  ...
  for (var i: f32 = 0.0; i < steps; i += 1.0) {
    ...
    var dense = clamp(calculate_density(samples.noise, samples.detail_noise, length(current_point - inner_sphere_point)), 0.05, 1.0);
    var cur_density = dense * getCoverage(current_point, distance_to_center, length(current_point - inner_sphere_point), samples, true) * cloudUniforms.density;
    cloud_density += cur_density;
    for(var k: f32 = 0.0; k < 2.0; k += 1.0){
      light = 0;
      var direction: vec3<f32>;

      direction = normalize(current_point - lightUniforms.lightPosition);

      let sun_ray_direction = normalize(direction);
      ...
      var theta = angleBetweenVectors(ray_direction, sun_ray_direction);
        ...
        var scale = getScale(sun_point, distance_to_center);
        light = mieScattering(theta) * lightUniforms.rayleighIntensity + samples.blue_noise.r * 0.05;
        if(cloud_density < 2.0){
        sun_transmittance = clamp(calculate_density(samples.noise, samples.detail_noise, scale), 0.05, 1.0) * getCoverage(sun_point, distance_to_center, distance_to_inner_sphere, samples, false) * cloudUniforms.sunDensity * clamp(lightness, lightclamp[0], lightclamp[1]);
