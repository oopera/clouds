...
@vertex fn vs(input: Input, @builtin(vertex_index) vertexIndex: u32) -> Output {
    ...
    output.vPosition = mPosition + displacement;
...
fn convertUVToNormal(uv: vec2<f32>) -> vec3<f32> {
  let u: f32 = uv.x * 2.005 * 3.14159265359; 
...
fn modifyDensity(originalDensity: f32, distance: f32, maxRadius: f32) -> f32 {
  let falloff = smoothstep(0.0, maxRadius, distance);
  return originalDensity * falloff;
}


fn cylinderSDF(p: vec3<f32>, center: vec3<f32>, radius: f32, height: f32) -> f32 {
  let d = vec2<f32>(length(p.xy - center.xy), p.z - center.z);
  return max(abs(d.x) - radius, abs(d.y) - height);
}
...
@fragment fn fs(output: Output) -> @location(0) vec4<f32> {
  ...
  let currentDiameter = mix(0.0, 0.25, clamp(timeSinceAnimationStart / 0.05, 0.0, 0.25));

  for (var depth: f32 = startDepth; depth < endDepth; depth += stepSize) {
    let texturePosition: vec3<f32> = rayOrigin + rayDirection * depth;

    coverage = getCoverage(texturePosition, ReMap(depth, startDepth, endDepth, .0, 3.0));
    noise = getNoise(texturePosition, vec3<f32>(1.0, 1.0, 1.0));
    noisedcoverage = computeNoise(coverage, noise);

    let sdfValue = cylinderSDF(texturePosition, selectedNormal, currentDiameter,  20); 
    if(sdfValue > 0.0){
    ...
    outputColor += density * sunDensity * highColor * light; 
